<script setup>
//Vista principal para añadir o editar un usuario regular por el usuario Registrador.
...
console.log("ADD/EDIT STORE", usersStore);
console.log("ADD/EDIT ALERT", alertStore);
...
const schema = Yup.object().shape({
  ...
  clave: Yup.string()
    ...
    //Contraseña opcional en edicion de usuario.
...
async function onSubmit(values) {
  ...
  try {
    ...
    if (user) {
      ...
      message = `Fiscal ${values.nombre} actualizado.`;
      await router.push("/users");
    } else {
      await usersStore.registerUser(values);
      message = `Fiscal ${values.nombre} agregado.`;
      await router.push("/users");
    }
    await router.push("/users");
...
<template>
  <div id="titulosOpciones">
    <h1><b>FISCALES</b></h1>
    <h5>{{ title }}, REVISAR CON DETALLE LOS DATOS ENVIADOS.</h5>
  </div>
  <br />
